...
function QiniuJsSDK() {
    ...
    this.uploader = function(op) {
        ...
        if(!op.qiniu_upurl) {
            throw 'qiniu_upurl is required!';
        }

        that.qiniu_upurl = op.qiniu_upurl;
        var option = {};
        ...
        uploader.bind('BeforeUpload', function(up, file) {
            ...
            if (uploader.runtime === 'html5' && chunk_size) {
                if (file.size < chunk_size) {
                    ...
                    ctx = '';
                    up.setOption({
                        'url': that.qiniu_upurl + '/mkblk/' + blockSize,
                        'multipart': false,
        ...
        uploader.bind('ChunkUploaded', function(up, file, info) {
            ...
            if (leftSize < chunk_size) {
                up.setOption({
                    'url': that.qiniu_upurl + '/mkblk/' + leftSize
                });
            }
        ...
        uploader.bind('FileUploaded', (function(_FileUploaded_Handler) {
            return function(up, file, info) {
                ...
                if(typeof info.response === 'string' && info.response.substr(0,5).toUpperCase() === '<PRE>'){
                    info.response = info.response.substr(5,info.response.length-5-5-1);
                }
                ...
                if (ctx) {
                    ...
                    var url = that.qiniu_upurl + '/mkfile/' + file.size + key + x_vars_url;
                    var ajax = that.createAjax();
