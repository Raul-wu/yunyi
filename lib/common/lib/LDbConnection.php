 ...
 * by lmt
 * */
class LDbConnection extends CDbConnection
{
    public $timeout = 10; //连接超时时间
    public $markDeadSeconds = 10; //如果从数据库连接失败 600秒内不再连接
    ...
    /**
     * @var slavesWrite 紧急情况 主数据库无法连接 启用从数据库 写功能.
     */
    public $slavesWrite = false;

    /**
     * @var masterRead 紧急情况 从数据库无法连接 启用主数据库 读功能.
     */
    ...
    {
        if ($this->enableSlave && !empty($this->slaves) && is_string($sql) && !$this->getCurrentTransaction()
            && self::isReadOperation($sql) && ($slave = $this->getSlave()))
        {
    ...
    private function _isDeadServer($c)
    {
        ...
        if ($cache && $cache->get('DeadServer1::' . $c) == 1)
        {
            return true;
        }
        return false;
